...
{
  course_pub_ = node_->create_publisher<ObservationCourseStampedMsg>("course", sensor_data_qos());
}
...
{
  position_pub_ =
    node_->create_publisher<ObservationPositionStampedMsg>("position", sensor_data_qos());
}
...
{
  auto callback = std::bind(&GPSPluginBase::process_nmea_, this, std::placeholders::_1);

  nmea_sub_ =
    node_->create_subscription<NmeaSentenceMsg>("gps/nmea_sentence", best_effort(10), callback);
}
...
void GPSPluginBase<CorePlugin>::init_odom_subscriber_()
{
  ...
  odom_sub_ =
    node_->create_subscription<OdometryMsg>("vehicle_controller/odom", best_effort(10), callback);
}
...
{
  if constexpr (std::is_same_v<CorePlugin, core::localisation::SingleAntennaGPSPlugin>) {
    plugin_->process_linear_speed(to_romea_duration(msg->header.stamp), msg->twist.twist.linear.x);
  }
}
