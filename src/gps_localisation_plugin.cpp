...
{
  auto gps = std::make_unique<GPSReceiver>(get_gps_gps_fix_eure(node_),
  ...
  plugin_ = std::make_unique<LocalisationGPSPlugin>(std::move(gps),
...
{
  plugin_->processLinearSpeed(to_romea_duration(msg->header.stamp),
...
//-----------------------------------------------------------------------------
void GPSLocalisationPlugin::publish_position_(const rclcpp::Time & stamp,
...
//-----------------------------------------------------------------------------
void GPSLocalisationPlugin::publish_course_(const rclcpp::Time & stamp,
                                            const std::string & frame_id)
{
